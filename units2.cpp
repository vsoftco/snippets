...
template <int M, int L, int T>
const Unit<M, L, T> operator+(const Unit<M, L, T>& lhs,
                              const Unit<M, L, T>& rhs) {
...
const Unit<M1 + M2, L1 + L2, T1 + T2> operator*(const Unit<M1, L1, T1>& lhs,
                                                const Unit<M2, L2, T2>& rhs) {
...
// can have more constants for all other units in terms of meter, second and
// kilogram feet,yard,inches,micrometer,hour,minute,grams,pounds, etc etc
// etc....
...
int main() {
    ...
    std::cout << "distanceinFeets = " << distanceinFeets.getValue()
              << std::endl;
